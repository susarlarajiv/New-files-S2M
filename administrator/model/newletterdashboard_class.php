...
class newletterSubsribedashboard
...
$fieldnames=array('title'=>mysql_real_escape_string($post['title']),
...

	{
      //print_r($res);exit;
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&err=newsletter subscribers created successfully");
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&ferr=blog creation failed");
...
$eventimage = $callConfig->freeimageUploadcomncode("banners",'image',"../uploads/blog/","../uploads/blog/thumbs/",$post['hdn_image'],'','');
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&err=newsletter subscribers updated successfully");
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&ferr=newsletter subscribers updation failed");
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&err=subscriber deleted successfully");
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&ferr=subscriber deletion failed");
...
$res=$callConfig->updateRecord(TPREFIX.TBL_NEWSLETTER_SUBSCRIBER,$fieldnames,'id',$_GET['id']);
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&err=subscriber Blocked successfully");
...

	{
		...
		$callConfig->headerRedirect("index.php?option=com_newsletters_subscribers&ferr=subscriber Blocked failed");
